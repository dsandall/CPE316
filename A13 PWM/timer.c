...
void TimerPWM_C2(float duty){
	//	PWM / compare mode
		TIM2->CCR2 = (uint32_t)(duty*autoreload); // capture/compare register #1 - 32/16 bits to compare with the count

		//TIM2->CCMR1 &= ~(0x7 << 12); //clear CCMR2
		TIM2->CCMR1 |= (0x7 << 12) ; // capture/compare mode register #2 - set TIM2 to PWM2 Mode (active when below CCR2)
